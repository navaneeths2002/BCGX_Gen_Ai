...
#reading the final data reposrt from task_1
final_report=pd.read_csv('final_financial_report.csv')
summary_report=pd.read_csv('Summary_final_financial_report.csv')

##creating chat bot using basic Rule
...
def financial_chatbot():
    ...
    if user_query == "What is the total revenue?":
        ...
        total_assets = final_report[(final_report['Year'] == fiscal_year) &(final_report['Compnay'] == company_input)]['Total Assets'].values[0]
    ...
    elif user_query.lower() == "What is the assets growth(%) ?":
...
while True:
    ...
    if user_input.lower() == "exit":
        ...
        if company_input not in ['Apple' , 'Microsoft' , 'Tesla']:
            print("Invalid company name please check and enter correct company name by starting the chatBot section again")
            break
        else:
            print("\nThe data for the fiscal year 2023, 2022, 2021 is curently available")
            fiscal_year=int(input("The fiscal year for the selected company : "))
            if fiscal_year not in [2023,2022,2021]:
                print("Please enter a valid fiscal year by starting the session again")
                break
    else:
